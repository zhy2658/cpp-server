...
| 类型 | 例子 | 处理方 | 特点 |
| :--- | :--- | :--- | :--- |
| **局内经济** | CS:GO 买枪、王者荣耀买装备 | **C++ Server (L4)** | 高频、极速、单局有效。内存校验余额，不读库。 |
| **局外经济** | 商城买皮肤、抽卡、充值 | **Golang Server** | 低频、事务安全、持久化。读写 MySQL/Redis。 |
| **带入资产** | 复活币、自带枪械 | **Golang -> C++** | 开局时通过 RPC 注入 C++ 内存；结算时回传消耗量。 |
